package org.seekloud.VideoMeeting.player.demo

import akka.actor.typed.{ActorRef, Behavior}
import akka.actor.typed.scaladsl.{Behaviors, StashBuffer, TimerScheduler}
import javafx.scene.canvas.GraphicsContext
import org.seekloud.VideoMeeting.player.core.PlayerGrabber
  ...
  def working(
    ...
    msg match {
      ...
      case msg: AddPicture =>
        ...
        val newImageQueue = imageQueue.get.enqueue(msg)
        working(id, grabActor, Some(newImageQueue), samplesQueue, pictureFinish, soundFinish)
        Behaviors.same

      case AddSamples(samples, ts) =>
        log.debug(s"VideoPlayer-$id got AddSample.")
        val newSamplesQueue = samplesQueue.get.enqueue(samples)
        working(id, grabActor, imageQueue, Some(newSamplesQueue), pictureFinish, soundFinish)
        Behaviors.same
